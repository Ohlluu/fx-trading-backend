...
class BearishProTraderGold:
                ...
                if current_candle_high > swing_level:
                    ...
                    if pips_above_swing >= 5.0 and wick_size >= 8.0 and current_price < swing_level:
                        ...
                        return {
                            ...
                            "description": f"🔥 LIVE Liquidity Grab at ${swing_level:.2f}! Price spiking to ${current_candle_high:.2f} ({pips_above_swing:.1f} pips above) rejecting {wick_size:.1f} pips DOWN NOW!"
                        }

            # Also check H4 resistance levels for current candle grabs
            resistance_levels = h4_levels.get("resistance_levels", [])
            for resistance in resistance_levels:
                if abs(resistance - current_price) < 30:
                    if current_candle_high > resistance:
                        ...
                        if pips_above >= 5.0 and wick_size >= 8.0 and current_price < resistance:
                            return {
                                ...
                                "description": f"🔥 LIVE Liquidity Grab at H4 resistance ${resistance:.2f}! Price spiking to ${current_candle_high:.2f} rejecting {wick_size:.1f} pips DOWN NOW!"
                            }
                ...
                if candle['high'] > swing_level:
                    ...
                    if pips_above_swing >= 5.0 and wick_size >= 8.0 and candle['close'] < swing_level:
                        ...
                        return {
                            ...
                            "rejection_size": round(wick_size, 1),
                            "pips_above": round(pips_above_swing, 1),
                            "description": f"🔥 Liquidity Grab at ${swing_level:.2f}! Price spiked to ${candle['high']:.2f} ({pips_above_swing:.1f} pips above) then rejected {wick_size:.1f} pips DOWN - {freshness}"
                        }
                    ...
                    if candle['high'] > resistance:
                        ...
                        if pips_above >= 5.0 and wick_size >= 8.0 and candle['close'] < resistance:
                            ...
                            return {
                                ...
                                "rejection_size": round(wick_size, 1),
                                "pips_above": round(pips_above, 1),
                                "description": f"🔥 Liquidity Grab at H4 resistance ${resistance:.2f}! Price spiked to ${candle['high']:.2f} then rejected {wick_size:.1f} pips - {freshness}"
                            }
    ...
    async def _detect_setup_pattern(self, h1_data: pd.DataFrame, h4_data: pd.DataFrame, h4_levels: Dict, current_price: float) -> Dict[str, Any]:
        ...
        # Example: BOS + BREAKDOWN_RETEST from same structural break should not both count
    ...
    def _dedupe_confluences(self, confluences: List[Dict]) -> Tuple[List[Dict], int]:
        ...
        - BOS + BREAKDOWN_RETEST (same structural break)
        - ORDER_BLOCK + SUPPLY_ZONE (same price level)
    ...
    def _find_targets(self, current_price: float, confluences: List[Dict],
        ...
        """
        Find target levels for Step 4 trade worthiness (BEARISH)
        ...
        # Deduplicate within 5 pips for Gold
        deduped_levels = []
        for level in levels:
            is_duplicate = False
            for existing in deduped_levels:
                if abs(level["price"] - existing["price"]) <= 5.0:
                    is_duplicate = True
                    break
            if not is_duplicate:
    ...
    def _assess_trade_worthiness(self, entry_state_data: Dict, confluences: List[Dict],
        ...
        """
        Step 4: Trade-worthiness gating (R:R + stop sanity + target feasibility) (BEARISH)
        ...
        # Only if: grade="TRADE"/"A+" AND stop <= 0.51 ATR AND entry_window="GOOD"
        ...
        feasibility_threshold = max(0.15 * atr, 0.30 * risk_distance)
        ...
        return {
            ...
            "reward_distance_tp1": round(reward_distance_tp1, 1) if reward_distance_tp1 is not None else None,
            "reward_distance_tp2": round(reward_distance_tp2, 1) if reward_distance_tp2 is not None else None,
    ...
    def _check_fvg(self, candles: pd.DataFrame, current_price: float) -> Dict[str, Any]:
        ...
        for idx, candle in recent_candles.iterrows():
            ...
            if candle['high'] >= nearest_fvg["bottom"]:
                ...
                # High touched FVG + closed significantly below (8+ pips)
                wick_size = candle['high'] - max(candle['open'], candle['close'])
    ...
    def _build_trade_plan(self, setup: Dict, h4_levels: Dict, current_price: float) -> Dict[str, Any]:
        ...
        if liquidity_grab or order_block:
            ...
            entry = key_level - 2
            entry_reason = f"Enter 2 pips below ${key_level:.2f} supply zone"
        else:
        ...
        if liquidity_grab and liquidity_grab_high:
            confluence_levels.append({
                ...
                "sl": liquidity_grab_high + 6,
                "reason": f"Above liquidity grab high (${liquidity_grab_high:.2f}) - stops already swept"
            })
        ...
        if supply_zone:
            confluence_levels.append({
                ...
                "level": key_level,
                "sl": key_level + 6,
                "reason": f"Above supply zone (${key_level:.2f}) with 6 pip buffer"
            })

        if order_block:
            # Parse OB top from description
            ob_desc = order_block.get("description", "")
            # Extract top from format: "Order Block at $X ($bottom-$top)"
            import re
            match = re.search(r'\$(\d+\.\d+)-\$(\d+\.\d+)', ob_desc)
            if match:
                ob_top = float(match.group(2))
                confluence_levels.append({
                    "name": "Order Block",
                    "level": ob_top,
                    "sl": ob_top + 6,
                    "reason": f"Above Order Block top (${ob_top:.2f}) with 6 pip buffer"
                })

        if fvg:
            # Get FVG level from setup data
            fvg_level = setup.get("fvg_zone", {}).get("top") if "fvg_zone" in setup else None
            if fvg_level:
                confluence_levels.append({
                    "name": "FVG",
                    "level": fvg_level,
                    "sl": fvg_level + 6,
                    "reason": f"Above FVG top (${fvg_level:.2f}) with 6 pip buffer"
                })

        if breakdown_retest:
            # Breakdown retest uses key_level
            ...
            if breakdown_level:
                confluence_levels.append({
                    ...
                    "sl": breakdown_level + 6,
                    "reason": f"Above breakdown level (${breakdown_level:.2f}) with 6 pip buffer"
                })
    ...
    def _check_dead_trade(self, entry_price: float, stop_price: float,
                         ...
                         direction: str, time_limit_hours: float = 6.0,
        ...
        Defaults for Gold: 6 hours, 0.3R minimum progress
        """
    ...
    def _check_early_exit(self, h1_data: pd.DataFrame, current_price: float) -> Dict:
        ...
        direction = setup.get("direction", "BEARISH")
        ...
        # Use configurable thresholds (Gold: 6h, 0.3R)
        dead_trade_check = self._check_dead_trade(
            ...
            direction=direction,
            time_limit_hours=6.0,  # Gold threshold
            min_progress_r=0.3
        )
    ...
    def _breakout_retest_invalidation(self, setup: Dict, state: str, key_level: float) -> List[Dict[str, Any]]:
        ...
        if state == "BREAKOUT_CONFIRMED":
            ...
            conditions.append({
                "condition": f"Price moves below ${runaway_threshold:.2f} without pullback",

...
class BullishProTraderEURUSD:
    ...
    def _calculate_atr(self, data: pd.DataFrame, period: int = 14) -> float:
        ...
        if data is None or len(data) < period:
            # Default fallback for EUR/USD: 20 pips * pip_size
            # EUR/USD pip_size = 0.0001 (quotes like 1.0850)
            return 20.0 * self.pip_size  # 20 pips = 0.0020 for EUR/USD
    ...
    def _determine_entry_state(self, confluences: List[Dict], grading: Dict,
        ...
        # SETUP: Location valid, waiting for trigger
            ...
            setup_state = "SETUP"
            entry_ready = False
            reason = f"Strong location at ${zone['midpoint']:.5f}; waiting for trigger (liquidity grab or retest)"
    ...
    def _find_targets(self, current_price: float, confluences: List[Dict],
        ...
        """
        Find target levels for Step 4 trade worthiness (BULLISH EUR/USD)
        ...
        opposing_types = ["SUPPLY_ZONE", "ORDER_BLOCK"]
        ...
        for conf in confluences:
            ...
            # For bullish: use bottom of supply zone (first resistance)
            if "level" in conf:
                level_price = conf["level"]
            elif "bottom" in conf and "top" in conf:
                level_price = conf["bottom"]  # Bottom of supply zone = first resistance hit
            else:
                continue

            # Only consider levels above current price (bullish targets up)
            ...
            levels.append({
                "price": level_price,
                "distance": self._to_pips(distance),  # Convert to pips for EUR/USD
                "source": "STRUCTURE",
                "type": conf.get("type", "UNKNOWN")
            })

        # 2. Find swing extremes from H4 data
        # For bullish: find swing highs (resistance levels above price)
        if h4_data is not None and not h4_data.empty and len(h4_data) >= 5:
            for i in range(2, len(h4_data) - 2):
                current_high = h4_data.iloc[i]['high']

                # Swing high: higher than 2 candles before and after
                is_swing_high = (
                    current_high > h4_data.iloc[i-1]['high'] and
                    current_high > h4_data.iloc[i-2]['high'] and
                    current_high > h4_data.iloc[i+1]['high'] and
                    current_high > h4_data.iloc[i+2]['high']
                )

                if is_swing_high and current_high > current_price:
                    distance = abs(current_price - current_high)
                    levels.append({
                        "price": current_high,
        ...
        # Deduplicate within 3 pips for EUR/USD
        deduped_levels = []
        for level in levels:
            is_duplicate = False
    ...
    def _assess_trade_worthiness(self, entry_state_data: Dict, confluences: List[Dict],
        ...
        """
        Step 4: Trade-worthiness gating (R:R + stop sanity + target feasibility) (BULLISH EUR/USD)
        ...
        # Gate 1: Stop Sanity (EUR/USD: <= 0.50 ATR)
        stop_atr_multiple = risk_distance / atr if atr > 0 else 999
        max_stop_atr = 0.50  # EUR/USD threshold
        ...
        can_use_atr_projection = (
            ...
            stop_atr_multiple <= (max_stop_atr * 0.85) and  # 0.425 for EUR/USD
        ...
        # Gate 2: Minimum R:R (EUR/USD: >= 1.6)
        min_rr_required = 1.6
    ...
    def _check_dead_trade(self, entry_price: float, stop_price: float,
        ...
        Defaults for FX (EUR/USD): 12 hours, 0.3R minimum progress
        """
    ...
    def _check_early_exit(self, h1_data: pd.DataFrame, current_price: float) -> Dict:
        ...
        dead_trade_check = self._check_dead_trade(
            ...
            direction=direction,
            time_limit_hours=12.0,  # FX threshold (EUR/USD)
            min_progress_r=0.3
        )
